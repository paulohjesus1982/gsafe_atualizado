...
class AddColumsToParalizacoesPremissas extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('paralizacoes_premissas', function (Blueprint $table) {
            $table->bigInteger('ppre_finalizado_por')->nullable();
            $table->foreign('ppre_finalizado_por')->references('usu_id')->on('usuarios');
    ...
    {
        Schema::table('paralizacoes_premissas', function (Blueprint $table) {
            $table->dropForeign(['ppre_finalizado_por']);
            $table->dropColumn('ppre_finalizado_por');
